...
export default function EditOrderPage() {
    ...
    React.useEffect(() => {
        ...
        const fetchOrder = async () => {
            try {
                ...
                toast({ variant: 'destructive', title: 'Алдаа', description: 'Үнийн саналын мэдээлэл татахад алдаа гарлаа.' });
            } finally {
                setIsLoading(false);
            }
        };
    ...
    async function onSubmit(values: FormValues) {
        ...
        try {
            ...
            toast({ title: 'Амжилттай', description: 'Үнийн саналын мэдээлэл шинэчлэгдлээ.' });
            router.push(`/quotes/${id}`);
        } catch (error) {
            console.error("Error updating order:", error);
            toast({ variant: 'destructive', title: 'Алдаа', description: 'Үнийн санал шинэчлэхэд алдаа гарлаа.' });
        } finally {
            ...
            <Form {...form}>
                 <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-8">
                    <Card className="max-w-4xl mx-auto">
                        <CardHeader>
                            <CardTitle>Үнийн санал засах</CardTitle>
                            <CardDescription>Үнийн саналын дугаар: {order?.orderNumber}</CardDescription>
                        </CardHeader>
                        <CardContent className="space-y-6">
                            <FormField
                                ...
                                render={({ field }) => (
                                    <FormItem>
                                        <FormLabel>Үнийн саналын статус</FormLabel>
                    ...
                    <div className="flex justify-end gap-2 max-w-4xl mx-auto">
                        <Button type="button" variant="outline" asChild>
                            <Link href={`/quotes/${id}`}>Цуцлах</Link>
                        </Button>
