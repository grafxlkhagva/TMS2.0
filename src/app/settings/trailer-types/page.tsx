...
export default function TrailerTypesPage() {
  const [items, setItems] = React.useState<TrailerType[]>([]);
  const [isLoading, setIsLoading] = React.useState(true);
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [itemToDelete, setItemToDelete] = React.useState<TrailerType | null>(null);
  const [itemToEdit, setItemToEdit] = React.useState<TrailerType | null>(null);
  ...
  const fetchItems = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const q = query(collection(db, "trailer_types"), orderBy("createdAt", "desc"));
  ...
  const handleAddNew = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await addDoc(collection(db, 'trailer_types'), {
        name: values.name,
        createdAt: serverTimestamp(),
      });
      toast({ title: 'Амжилттай', description: 'Шинэ тэвшний төрөл нэмэгдлээ.' });
      form.reset({ name: '' });
      fetchItems();
    } catch (error) {
  ...
  const handleUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    setIsSubmitting(true);
    try {
      const docRef = doc(db, 'trailer_types', itemToEdit.id);
      await updateDoc(docRef, { name: values.name });
      toast({ title: 'Амжилттай', description: 'Тэвшний төрлийн нэр засагдлаа.' });
      setItemToEdit(null);
      fetchItems();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'trailer_types', itemToDelete.id));
      toast({ title: 'Амжилттай', description: 'Тэвшний төрөл устгагдлаа.' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6">
       <div className="mb-6">
        ...
        <h1 className="text-3xl font-headline font-bold">Тэвшний төрөл</h1>
        <p className="text-muted-foreground">
          Тэвшний төрлүүдийг удирдах лавлах сан.
        </p>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    <FormControl>
                      <Input placeholder="Жишээ нь: Хагас чиргүүл" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleUpdate)}>
              <DialogHeader>
                ...
                <DialogDescription>
                  Тэвшний төрлийн нэрийг өөрчлөх.
                </DialogDescription>
              </DialogHeader>
              <div className="py-4">
                 <FormField
