...
export default function VehicleTypesPage() {
  const [items, setItems] = React.useState<VehicleType[]>([]);
  const [isLoading, setIsLoading] = React.useState(true);
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [itemToDelete, setItemToDelete] = React.useState<VehicleType | null>(null);
  const [itemToEdit, setItemToEdit] = React.useState<VehicleType | null>(null);
  ...
  const fetchItems = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const q = query(collection(db, "vehicle_types"), orderBy("createdAt", "desc"));
  ...
  const handleAddNew = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({ title: 'Амжилттай', description: 'Шинэ машины төрөл нэмэгдлээ.' });
      form.reset({ name: '' });
      fetchItems();
    } catch (error) {
  ...
  const handleUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    setIsSubmitting(true);
    try {
      const docRef = doc(db, 'vehicle_types', itemToEdit.id);
      await updateDoc(docRef, { name: values.name });
      toast({ title: 'Амжилттай', description: 'Машины төрлийн нэр засагдлаа.' });
      setItemToEdit(null);
      fetchItems();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'vehicle_types', itemToDelete.id));
      toast({ title: 'Амжилттай', description: 'Машины төрөл устгагдлаа.' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6">
       <div className="mb-6">
        ...
        <h1 className="text-3xl font-headline font-bold">Машины төрөл</h1>
        <p className="text-muted-foreground">
          Машины төрлүүдийг удирдах лавлах сан.
        </p>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    <FormControl>
                      <Input placeholder="Жишээ нь: Хүнд даацын" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
