...
export default function RegionsPage() {
  ...
  const [itemToDelete, setItemToDelete] = React.useState<Region | null>(null);
  const [itemToEdit, setItemToEdit] = React.useState<Region | null>(null);
  ...
  const handleAddNew = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      ...
      toast({ title: 'Амжилттай', description: 'Шинэ бүс нэмэгдлээ.' });
      form.reset({ name: '' });
      fetchItems();
    } catch (error) {
  ...
  const handleUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    setIsSubmitting(true);
    try {
      const docRef = doc(db, 'regions', itemToEdit.id);
      await updateDoc(docRef, { name: values.name });
      toast({ title: 'Амжилттай', description: 'Бүсийн нэр засагдлаа.' });
      setItemToEdit(null);
      fetchItems();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'regions', itemToDelete.id));
      toast({ title: 'Амжилттай', description: 'Бүс устгагдлаа.' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6">
       <div className="mb-6">
        ...
        <h1 className="text-3xl font-headline font-bold">Бүс нутаг</h1>
        <p className="text-muted-foreground">
          Аймаг, нийслэлийн нэрийг удирдах лавлах сан.
        </p>
      </div>

      <Card>
        <CardHeader>
          <CardTitle>Шинэ бүс нэмэх</CardTitle>
        </CardHeader>
        <CardContent>
          <Form {...form}>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    <FormControl>
                      <Input placeholder="Жишээ нь: Улаанбаатар" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <Card className="mt-6">
        <CardHeader>
          <CardTitle>Бүсүүдийн жагсаалт</CardTitle>
          <CardDescription>Нийт {regions.length} бүс байна.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
            ...
            <TableBody>
              {isLoading ? (
                ...
                <TableRow>
                  <TableCell colSpan={3} className="text-center h-24">Бүртгэлтэй бүс олдсонгүй.</TableCell>
                </TableRow>
              )}
            </TableBody>
          </Table>
        ...
        <DialogContent>
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleUpdate)}>
              <DialogHeader>
                <DialogTitle>Бүс засах</DialogTitle>
                <DialogDescription>
                  Бүс нутгийн нэрийг өөрчлөх.
                </DialogDescription>
              </DialogHeader>
              <div className="py-4">
                 <FormField
                  ...
                  render={({ field }) => (
                    <FormItem>
                      <FormControl>
                        <Input placeholder="Бүсийн нэр" {...field} />
                      </FormControl>
                      <FormMessage />
                    </FormItem>
                  )}
            ...
            <AlertDialogContent>
                <AlertDialogHeader>
                    ...
                    <AlertDialogDescription>
                        "{itemToDelete?.name}" бүсийг устгах гэж байна. Энэ үйлдлийг буцаах боломжгүй.
                    </AlertDialogDescription>
                </AlertDialogHeader>
                <AlertDialogFooter>
