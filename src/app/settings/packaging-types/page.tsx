...
export default function PackagingTypesPage() {
  const [items, setItems] = React.useState<PackagingType[]>([]);
  const [isLoading, setIsLoading] = React.useState(true);
  const [isSubmitting, setIsSubmitting] = React.useState(false);
  const [itemToDelete, setItemToDelete] = React.useState<PackagingType | null>(null);
  const [itemToEdit, setItemToEdit] = React.useState<PackagingType | null>(null);
  ...
  const fetchItems = React.useCallback(async () => {
    setIsLoading(true);
    try {
      const q = query(collection(db, "packaging_types"), orderBy("createdAt", "desc"));
  ...
  const handleAddNew = async (values: z.infer<typeof formSchema>) => {
    ...
    try {
      await addDoc(collection(db, 'packaging_types'), {
        name: values.name,
        createdAt: serverTimestamp(),
      });
      toast({ title: 'Амжилттай', description: 'Шинэ баглаа боодлын төрөл нэмэгдлээ.' });
      form.reset({ name: '' });
      fetchItems();
    } catch (error) {
  ...
  const handleUpdate = async (values: z.infer<typeof formSchema>) => {
    ...
    setIsSubmitting(true);
    try {
      const docRef = doc(db, 'packaging_types', itemToEdit.id);
      await updateDoc(docRef, { name: values.name });
      toast({ title: 'Амжилттай', description: 'Баглаа боодлын төрлийн нэр засагдлаа.' });
      setItemToEdit(null);
      fetchItems();
    } catch (error) {
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteDoc(doc(db, 'packaging_types', itemToDelete.id));
      toast({ title: 'Амжилттай', description: 'Баглаа боодлын төрөл устгагдлаа.' });
      fetchItems();
    } catch (error) {
  ...
  return (
    <div className="container mx-auto py-6">
       <div className="mb-6">
        ...
        <h1 className="text-3xl font-headline font-bold">Баглаа боодлын төрөл</h1>
        <p className="text-muted-foreground">
          Ачааны баглаа боодлын төрлийн лавлах сан.
        </p>
      </div>
              ...
              <FormField
                ...
                render={({ field }) => (
                  <FormItem className="flex-1">
                    <FormControl>
                      <Input placeholder="Жишээ нь: Хайрцаг" {...field} />
                    </FormControl>
                    <FormMessage />
                  </FormItem>
                )}
      ...
      <Card className="mt-6">
        <CardHeader>
          ...
          <CardDescription>Нийт {items.length} төрөл байна.</CardDescription>
        </CardHeader>
        <CardContent>
          <Table>
            <TableHeader>
          ...
          <Form {...editForm}>
            <form onSubmit={editForm.handleSubmit(handleUpdate)}>
              <DialogHeader>
                ...
                <DialogDescription>
                  Баглаа боодлын төрлийн нэрийг өөрчлөх.
                </DialogDescription>
              </DialogHeader>
              <div className="py-4">
                 <FormField
